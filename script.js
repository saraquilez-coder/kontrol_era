...
function finalizarTodo() {
    if(confirm("¿FINALIZAR INTERVENCIÓN TOTAL? Se limpiará todo el panel.")) {
        // Al finalizar todo, simplemente limpiamos lo activo. 
        // Los datos ya se han ido guardando en el historial cada vez que un equipo salía.
    ...
    eqs.forEach((e, i) => {
        ...
        let cardHtml = `
            <div id="card-${i}" class="card ${e.activo?'':'fuera'} ${e.alerta?'alerta-equipo':''}">
                ...
                <div class="card-name">${e.n} ${e.reactivado === "SÍ" ? '<small>(Reactivado)</small>' : ''}</div>
                ...
                <div class="seccion">
                    ...
                    <div class="dato"><span>Presión Entrada:</span> <span class="val">${Math.round(e.pE)} bar</span></div>
                ...
                ` : `
                    <button class="btn btn-blue" style="background:#28a745" onclick="reactivarEquipo(${i})">RE-ACTIVAR (NUEVA ENTRADA)</button>
                `}
            </div>`;
...
// Al pulsar FIN EQUIPO, guardamos el registro en el historial
function setEstado(i, activo) { 
    if (!activo) {
        let ah = Date.now();
        eqs[i].hSalida = formatHora(ah);
        eqs[i].tAcumuladoPrevio += (ah - eqs[i].tI);
        eqs[i].activo = false;
        
        // GUARDAR AUTOMÁTICAMENTE EN HISTORIAL ESTE TRAMO
...
function reactivarEquipo(i) {
    ...
    if(b) {
        let ah = Date.now();
        // Reiniciamos los valores para la nueva entrada pero conservamos el nombre y profesionales
    ...
    historial.slice().reverse().forEach(reg => {
        ...
        html += `
            <div style="background:white; padding:10px; margin-bottom:5px; border-left:5px solid red; font-size:0.8rem; color:#000;">
                <b>${reg.fecha}</b> - ${e.n} (${e.hE} a ${e.hSalida})<br>
                Misión: ${e.obj} / Reactivado: ${e.reactivado}
            </div>
        `;
    });
...
function exportarTodoCSV() {
    ...
    historial.forEach(reg => {
        ...
        let fila = [
            ...
            (e.pE - e.pA),
            Math.round(e.rMed),
            formatTimeMS(e.activo ? (Date.now() - e.tI) : (e.tAcumuladoPrevio)),
            e.reactivado
        ].join(";");
        csvContent += fila + "\n";
    });
    ...
    link.setAttribute("download", `PARTE_INTERVENCION_${Date.now()}.csv`);
